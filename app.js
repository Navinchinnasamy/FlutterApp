...
function renderStats(){ $("#total-items").textContent=items.length; $("#low-items").textContent=items.filter(i=>i.status==="low").length; $("#expiring-items").textContent=items.filter(i=>daysUntil(i.date)<=7).length; $("#nav-count").textContent=shopping.filter(i=>!i.done).length; }
function renderSidebarCategories(){
  const categories = [...new Set(["Produce", "Dairy", "Pantry", "Freezer", ...items.map(item => item.category).filter(Boolean)])];
...
function renderCategories(){ const cats=["Produce","Dairy","Pantry","Freezer"]; const colors=["#8dbb94","#84b6c0","#dfbf76","#aa9bd0"]; $("#category-list").innerHTML=cats.map((c,i)=>{const n=items.filter(x=>x.category===c).length; return `<div class="category-row"><span class="cat-name">${c}</span><span class="cat-track"><i style="width:${Math.max(n*16,8)}%;background:${colors[i]}"></i></span><span class="cat-count">${n}</span></div>`}).join(""); $(".category-total strong").textContent=items.length; }
function attentionMarkup(){ const list=items.filter(i=>i.status!=="ok" || daysUntil(i.date)<=7).slice(0,4); $("#attention-list").innerHTML=list.length?list.map(i=>`<div class="attention-row"><span class="food-icon">${i.icon}</span><span class="food-name"><strong>${i.name}</strong><small>${i.quantity} · Best before ${dateLabel(i.date)}</small></span>${statusPill(i.status)}<span class="quantity">${i.category}</span></div>`).join(""):'<p class="empty-state">Everything looks nicely stocked.</p>'; }
function shoppingMarkup(target="#shopping-preview"){ const list=shopping.slice(0,target==="#shopping-preview"?3:shopping.length); $(target).innerHTML=list.length?list.map(i=>`<div class="shopping-row ${i.done?"done":""}"><button class="check" data-shop="${i.id}" aria-label="Mark ${i.name}">${i.done?"✓":""}</button><span class="food-name"><strong>${i.icon} &nbsp;${i.name}</strong><small>${i.note}${i.category?` · ${i.category}`:""}</small></span><span class="list-person">${i.who}</span></div>`).join(""):'<p class="empty-state">Your shopping list is empty.</p>'; }
function renderInventory(){ const query=($("#search-input")?.value||"").toLowerCase(); let filtered=items.filter(i=>i.name.toLowerCase().includes(query)); if(activeFilter==="low")filtered=filtered.filter(i=>i.status==="low"); if(activeFilter==="expiring")filtered=filtered.filter(i=>daysUntil(i.date)<=7); $("#inventory-list").innerHTML=filtered.map(i=>`<div class="inventory-row"><span class="item-name"><span class="food-icon">${i.icon}</span><span><strong>${i.name}</strong><small>${i.quantity}</small></span></span><span class="inventory-cell">${i.category}</span><span class="inventory-cell">${i.quantity}</span><span class="inventory-cell">${dateLabel(i.date)}</span>${statusPill(i.status)}<button class="row-actions" data-delete="${i.id}">•••</button></div>`).join("") || '<p class="empty-state">No items match that filter.</p>'; }
function renderShopping(){ const done=shopping.filter(i=>i.done).length; $("#shopping-progress-text").textContent=`${done} of ${shopping.length} items picked up`; $("#progress-bar").style.width=shopping.length?`${done/shopping.length*100}%`:"0%"; shoppingMarkup("#shopping-list-full"); }
...
function openModal(target = "inventory"){ modalTarget = target; $("#modal-backdrop").classList.remove("hidden"); $("#item-form").reset(); setTimeout(()=>$("#item-form input").focus(),50); }
function closeModal(){ $("#modal-backdrop").classList.add("hidden"); }
document.addEventListener("click",async e=>{ const view=e.target.closest("[data-view]"); if(view)showView(view.dataset.view); const cat=e.target.closest("[data-category]"); if(cat){showView("inventory"); $("#search-input").value=""; activeFilter="all"; renderInventory();} const shop=e.target.closest("[data-shop]"); if(shop){const item=shopping.find(i=>i.id==shop.dataset.shop); const wasDone=item.done; item.done=!item.done; item.updatedAt=now(); let purchased; if(item.done&&!wasDone){const timestamp=now();purchased={id:Date.now(),shoppingId:item.id,createdAt:timestamp,updatedAt:timestamp,name:item.name,category:item.category||"Pantry",quantity:item.note,date:item.date||"2026-09-30",icon:item.icon||"🛒",status:"ok"};items.unshift(purchased);} if(!item.done&&wasDone){const matching=items.find(i=>i.shoppingId===item.id);if(matching)items=items.filter(i=>i!==matching);} try{await save();render();}catch(error){item.done=wasDone;if(purchased)items=items.filter(i=>i!==purchased);showDataError(`Could not save that change: ${error.message}`);}} const del=e.target.closest("[data-delete]"); if(del&&confirm("Remove this item from your inventory?")){const removed=items.find(i=>i.id==del.dataset.delete);items=items.filter(i=>i.id!=del.dataset.delete);try{await save();render();}catch(error){if(removed)items.push(removed);showDataError(`Could not save that change: ${error.message}`);}} const addButton=e.target.closest("#open-add, #open-add-inventory, #open-add-shopping, #quick-add-list"); if(addButton)openModal(addButton.id==="open-add-shopping"||addButton.id==="quick-add-list"?"shopping":"inventory"); if(e.target.id==="close-modal"||e.target.id==="cancel-modal"||e.target.id==="modal-backdrop")closeModal();});
$("#item-form").addEventListener("submit",async e=>{e.preventDefault();const data=new FormData(e.target);const name=data.get("name").trim();if(!name)return;const date=data.get("date")||"2026-09-30";const timestamp=now();let added;if(modalTarget==="shopping"){added={id:Date.now(),createdAt:timestamp,updatedAt:timestamp,name,note:data.get("quantity"),quantity:data.get("quantity"),category:data.get("category"),date,icon:"🛒",done:false,who:"Navin"};shopping.unshift(added);}else{added={id:Date.now(),createdAt:timestamp,updatedAt:timestamp,name,category:data.get("category"),quantity:data.get("quantity"),date,icon:"🛒",status:"ok"};items.unshift(added);}try{await save();render();closeModal();}catch(error){if(modalTarget==="shopping")shopping=shopping.filter(i=>i!==added);else items=items.filter(i=>i!==added);showDataError(`Could not save this item: ${error.message}`);}});
